...
export default function AddToPot({ setModal, potData }: FormProps) {
  const [value, setValue] = useState<number | string>("");
  const { id, amount, target, color } = potData;

  const originalPercentage = Number((amount / target).toFixed(2)) * 100;
  const originalBarWidth =
    originalPercentage <= 100 ? `${originalPercentage}%` : "100%";

  const newPercentage = Number((Number(value) / target).toFixed(2)) * 100;
  const newBarWidth =
    newPercentage + originalPercentage > 100
      ? `${100 - originalPercentage}%`
      : `${newPercentage}%`;
  ...
  async function addToPot(e) {
    ...
    const response = await fetch("http://localhost:3000/pots/addToPot", {
      method: "PUT",
      credentials: "include",
      headers: {
    ...
    <>
      ...
      >
        <div className="flex w-full justify-between items-center">
          <div className="text-gray-900 text-preset-2">Add to Pot</div>
          <button
            onClick={() => {
              setModal("");
            }}
