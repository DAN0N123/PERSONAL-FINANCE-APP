...
export default function Bills() {
  const { data: bills, error, isLoading } = useSWR<BillType[]>(
      ...
      <div className="flex flex-col gap-[12px] xl:gap-[24px]">
        <div className="relative isolate flex flex-col overflow-y-clip">
          ...
          <div className="bg-beige-100 flex justify-between pl-[16px] pr-[16px] pt-[20px] pb-[20px] rounded-[8px] z-20">
            ...
            <p className="text-preset-4-bold text-gray-900"> ${paidBills && Math.round(paidBills.reduce( (acc, bill) => acc + bill.amount, 0))}</p>
          </div>
        </div>
        <div className="relative isolate flex flex-col overflow-y-clip">
          ...
          <div className="bg-beige-100 flex justify-between pl-[16px] pr-[16px] pt-[20px] pb-[20px] rounded-[8px] z-20">
            ...
            <p className="text-preset-4-bold text-gray-900"> ${upcomingBills && Math.round(upcomingBills.reduce( (acc, bill) => acc + bill.amount, 0))}</p>
          </div>
        </div>
        <div className="relative isolate flex flex-col overflow-y-clip">
          ...
          <div className="bg-beige-100 flex justify-between pl-[16px] pr-[16px] pt-[20px] pb-[20px] rounded-[8px] z-20">
            ...
            <p className="text-preset-4-bold text-gray-900"> ${dueBills && Math.round(dueBills.reduce( (acc, bill) => acc + bill.amount, 0))}</p>
          </div>
        </div>
      </div>
    </div>
